					  ...
					  unsigned int out_cnt)
{
	...
	case MMC_RPMB_WRITE_KEY:
		...
		/* Write request */
		multi_cmd->cmds[0].opcode = MMC_WRITE_MULTIPLE_BLOCK;
		multi_cmd->cmds[0].arg = 0x0;
		multi_cmd->cmds[0].flags = MMC_RSP_SPI_R1 | MMC_RSP_R1 | MMC_CMD_ADTC;
		multi_cmd->cmds[0].blksz = 512;
		multi_cmd->cmds[0].blocks = 1;
		multi_cmd->cmds[0].write_flag = (1<<31);
		multi_cmd->cmds[0].data_ptr = (uintptr_t)frame_in;

		/* Result request */
		memset(frame_out, 0, sizeof(*frame_out));
		frame_out->req_resp = htobe16(MMC_RPMB_READ_RESP);
		multi_cmd->cmds[1].write_flag = 1;
		multi_cmd->cmds[1].arg = 0x0;
		multi_cmd->cmds[1].blksz = 512;
		multi_cmd->cmds[1].flags = MMC_RSP_SPI_R1 | MMC_RSP_R1 | MMC_CMD_ADTC;
		multi_cmd->cmds[1].opcode   = MMC_WRITE_MULTIPLE_BLOCK;
		multi_cmd->cmds[1].blocks   = out_cnt;
		multi_cmd->cmds[1].data_ptr = (uintptr_t)frame_out;

		/* Get response */
		multi_cmd->cmds[2].write_flag = 0;
		multi_cmd->cmds[2].arg = 0x0;
		multi_cmd->cmds[2].blksz = 512;
		multi_cmd->cmds[2].flags = MMC_RSP_SPI_R1 | MMC_RSP_R1 | MMC_CMD_ADTC;
		multi_cmd->cmds[2].opcode   = MMC_READ_MULTIPLE_BLOCK;
		multi_cmd->cmds[2].blocks   = out_cnt;
		multi_cmd->cmds[2].data_ptr = (uintptr_t)frame_out;

		err = ioctl(fd, MMC_IOC_MULTI_CMD, multi_cmd);
		if (err < 0) {
			err = -errno;
			free(multi_cmd);
			goto out;
	...
	case MMC_RPMB_READ:
		...
		/* Get response */
		multi_cmd->cmds[1].write_flag = 0;
		multi_cmd->cmds[1].arg = 0x0;
		multi_cmd->cmds[1].blksz = 512;
		multi_cmd->cmds[1].flags = MMC_RSP_SPI_R1 | MMC_RSP_R1 | MMC_CMD_ADTC;
		multi_cmd->cmds[1].opcode   = MMC_READ_MULTIPLE_BLOCK;
		multi_cmd->cmds[1].blocks   = out_cnt;
		multi_cmd->cmds[1].data_ptr = (uintptr_t)frame_out;
